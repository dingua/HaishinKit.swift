...
//  Shortcut
//
//  Created by Mejdi Lassidi on 3/13/19.
...
class KVVideoManager: NSObject {
    ...
    func merge(video:AVAsset, withBackgroundMusic music:AVAsset, completion:@escaping Completion) -> Void {
        ...
        if music.tracks(withMediaType: AVMediaType.audio).isEmpty == false {
        ...
        let path = NSTemporaryDirectory().appending("\(videoTitle).mp4")
    ...
    private func doMerge(arrayVideos:[AVAsset], animation:Bool, completion:@escaping Completion) -> Void {
        ...
        for videoAsset in arrayVideos {
            ...
            // Get audio track
            var audioTrack:AVAssetTrack?
            if videoAsset.tracks(withMediaType: AVMediaType.audio).isEmpty == false {
